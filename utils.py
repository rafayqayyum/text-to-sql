...
def generate_schema(text,dbtype='mysql'):
  prompt="Query:\n"+text+"\n\nOnly Query:"
  tokens_discarded=0
  try:
    message=[{"role":"system",'content':"You are given a natural language query. You convert the natural language query to SQL query for "+dbtype+" database to generate the schema. Only the schema is returned."},
             {"role":"user",'content':prompt}]
    response = openai.ChatCompletion.create(
      model="gpt-3.5-turbo",
      messages=message,
